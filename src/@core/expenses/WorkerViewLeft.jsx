...
const UserViewLeft = (props) => {
  ...
  const initialFormData = { worker_id: '', name: '', status: '', role: '', mobile_number: '', site_id: '' }
  const [formData, setFormData] = useState(initialFormData)
  const [wages, setWages] = useState('')


  // console.log("WORKER ID", wages)
  ...
  const fetchUserData = async () => {
    ...
    try {
      ...
      const data = await AxiosPost('fetchWorker.php', { worker_id: props.selectedWorker })
      console.log(data)
      if (data.success) {
        setData(data.worker)
        console.log("Data", data.worker);
        setFormData({ ...formData, ...data.worker })
      }
  ...
  const fetchWages = async () => {
    setIsLoading(true);
    try {
      const data = await AxiosPost('fetchWages.php', { worker_id: props.selectedWorker })
  ...
  if (data) {
    return (
      <Grid container spacing={6}>
        <Grid item xs={12}>
          <Card>
            <Button size='small' color='secondary' sx={{ m: 3, mb: 0 }} variant="contained" onClick={() => props.setSelectedWorker(null)}>
              Go back
            </Button>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Box sx={{ mr: 8, display: 'flex', alignItems: 'center' }}>
                  ...
                  <div>
                    ...
                    <Typography variant='body'>Base Price</Typography>
                  </div>
                </Box>
                {/* <Box sx={{ display: 'flex', alignItems: 'center' }}>
                  ...
                  <div>
                    ...
                    <Typography variant='body'>Task Pending</Typography>
                  </div>
                </Box> */}
              </Box>
            </CardContent>
            ...
            <CardContent>
              ...
              <Box sx={{ pt: 2, pb: 1 }}>
                ...
                <Box sx={{ display: 'flex', mb: 2.7 }}>
                  ...
                  <Typography variant='body'>{data.site_id}</Typography>
                </Box>
                {/* <Box sx={{ display: 'flex', mb: 2.7 }}>
                  <Typography sx={{ mr: 2, fontWeight: 500, fontSize: '0.875rem' }}>Joined On:</Typography>
                  <Typography variant='body'>{data.created_at}</Typography>
                </Box> */}
              </Box>
            </CardContent>
