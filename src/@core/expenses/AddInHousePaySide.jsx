...
const AddInHousePaySide = ({ open, toggle, fetchExpensesInHouse }) => {
  ...
  const onSubmit = async (inputdata) => {
    try {
      ...
      const data = await AxiosPost('addLabourExp.php', { ...values, orgId: user.orgId, site_id: selectedSite, payment_reason: selectedExpenseReason, date: date, vendor_id: null, paymentType: paymentType });

      if (data.success) {
        await fetchExpensesInHouse();
  ...
  const calculateAmount = () => {
    if (values.worker !== '' && values.startTime !== '' && values.endTime !== '') {
      ...
      if (selectedWorker) {
        ...
        let timeDiffInHours = endTime.diff(startTime, 'hours', true);

        // Handle time difference when endTime is before startTime (e.g., overnight shifts)
  ...
  const fetchWorkers = async () => {
    try {
      const data = await AxiosPost('fetchInHouseWorkers.php', { orgId: user.orgId })
      console.log(data)
      if (data.success) {
  ...
  return (
    ...
    >
      <Header>
        <Typography variant='h6'>Add In House Pay</Typography>
      ...
      <Box component='form' sx={{ p: 5 }}>
        ...
        <Typography sx={{ mb: 4 }}>In House Payment for Site: <strong> {siteName}</strong></Typography>
...
export default AddInHousePaySide;
